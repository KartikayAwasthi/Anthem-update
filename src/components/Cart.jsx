...
const Cart = ({ isOpen, onClose }) => {
  ...
  const CartItemImage = ({ item, className, isOverview = false }) => {
    ...
    // Debug logging
    console.log(`CartItemImage - Item: ${item.name}, Image Path: ${item.image}, Current Src: ${imageSrc}`);

    const handleError = () => {
      if (!hasError && imageSrc !== '/fallback.jpg') {
        console.error(`❌ Image failed for ${item.name}, attempted path: ${imageSrc}, using fallback`);
    ...
    const handleLoad = () => {
      console.log(`✅ Image loaded successfully for ${item.name} from path: ${imageSrc}`);
      if (!isOverview) {
